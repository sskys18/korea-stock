...
//!
//! USDM Futures(`/fapi/v1/*`) 공개 엔드포인트. 가격·수량은 정밀도 보존 위해 String.

use serde::Deserialize;

use crate::global::binance::client::{ApiCall, BinanceClient};
use crate::global::binance::error::{BinanceError, Result};
...
/// 캔들(봉) 1건. Binance klines는 이종 배열이라 내부 [`KlineRaw`]로 받아 변환한다.
#[derive(Debug, Clone)]
pub struct Kline {
    /// 봉 시작 시각 (epoch ms).
    pub open_time: i64,
...
impl<'a> Market<'a> {
    ...
    /// 전체 심볼 목록. KR 종목만 보려면 [`crate::global::binance::KR_SYMBOLS`]로 필터.
    ...
    /// 단일 심볼 정보 조회 (없으면 [`BinanceError::Decode`]).
    pub async fn market_info(&self, symbol: &str) -> Result<SymbolInfo> {
        self.markets()
            .await?
            .into_iter()
            .find(|s| s.symbol == symbol)
            .ok_or_else(|| BinanceError::Decode(format!("symbol not found: {symbol}")))
    }
}
