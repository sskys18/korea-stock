...
use crate::global::aster::config::AsterConfig;
...
/// HTTP 응답 — Aster fapi는 envelope 없이 페이로드를 직접 반환한다(Binance 호환).
...
/// Aster fapi(Binance 호환) 클라이언트.
///
/// 시세 액세서(`market`)는 키 없이, 거래 액세서(`trade`)는 HMAC-SHA256 서명으로
/// 호출한다. 매 서명 요청마다 `timestamp`를 새로 찍는다.
///
/// **`Accept: application/json` 기본 헤더:** Aster CDN은 Accept 헤더가 없으면
/// 일부 엔드포인트(예: premiumIndex)에 스키마 문서 스텁(비-JSON)을 반환한다.
/// 헤더를 고정해 실제 시세 JSON을 받도록 보장한다(Binance 어댑터에는 불필요한 차이).
...
impl AsterClient {
    ...
    /// 시세 도메인 액세서 (키 불필요).
    pub fn market(&self) -> crate::global::aster::market::Market<'_> {
        crate::global::aster::market::Market::new(self)
    }

    /// 거래·계좌 도메인 액세서 (HMAC 서명).
    pub fn trade(&self) -> crate::global::aster::trade::Trade<'_> {
        crate::global::aster::trade::Trade::new(self)
    }
    ...
    /// HMAC-SHA256(secret, payload) → hex 소문자. Binance 호환 서명.
    fn sign(secret: &str, payload: &str) -> Result<String> {
        let mut mac = HmacSha256::new_from_slice(secret.as_bytes())
            .map_err(|e| AsterError::Sign(e.to_string()))?;
    ...
    pub(crate) async fn call(&self, c: ApiCall) -> Result<RawResponse> {
        ...
        loop {
            match self.call_once(&c).await {
                Err(AsterError::Api { status, .. }) if (status == 429 || status == 418) && attempt < MAX_RETRIES => {
...
/// Aster 에러 본문 `{code,msg}` 파싱(Binance 호환). 비-JSON이면 code=0 + 원문.
...
/// Aster fapi 파라미터 값은 영숫자·심볼 위주라 대부분 그대로지만, 안전을 위해 인코딩한다.
...
mod tests {
    ...
    // Aster fapi의 서명은 Binance와 바이트 동일(같은 secret/payload → 같은 hex)이므로
    // 이 벡터로 서명기 정확성을 네트워크 없이 검증한다 — 거래 안전의 핵심.
    #[test]
    fn hmac_matches_binance_doc_vector() {
        ...
        let sig = AsterClient::sign(secret, payload).unwrap();
        assert_eq!(
            sig,
    ...
    // Aster V1(Legacy) fapi 공식 문서의 worked HMAC-SHA256 예제 벡터.
    // 출처: github.com/asterdex/api-docs V1 futures API "SIGNED Endpoint" 섹션.
    // Aster 문서가 명시한 입력→서명을 바이트 대조해 Binance 호환임을 이중 확인.
    #[test]
    fn hmac_matches_aster_doc_vector() {
    ...
    fn encode_query_preserves_order() {
        ...
        assert_eq!(
            AsterClient::encode_query(&pairs),
