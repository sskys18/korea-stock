...
//!
//! ```ignore
//! use korea_stock::global::mexc::{MexcClient, MexcConfig, SAMSUNG};
//! use korea_stock::global::mexc::trade::{OrderRequest, Side, OpenType};
//!
//! // 시세 — 키 불필요.
...
/// [`crate::KrStock`] → MEXC 심볼(`*STOCK_USDT`). 미상장 종목은 `None`.
pub fn symbol(stock: crate::KrStock) -> Option<&'static str> {
    ...
    Some(match stock {
        ...
        Kospi200 => return None, // MEXC는 KR 지수 perp 미상장
    })
}
