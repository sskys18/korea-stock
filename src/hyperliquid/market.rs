...
///
/// universe 재정렬 시 하드코딩 상수([`crate::hyperliquid::SAMSUNG_ASSET`] 등)가
...
impl<'a> Market<'a> {
    ...
    /// perp 메타(universe). `dex`는 빌더 네임스페이스(예 [`crate::hyperliquid::DEX`]).
    ...
    /// 의한 wrong-instrument 사고를 막는다. [`crate::hyperliquid::trade::Trade::place_by_coin`]가
    /// 내부적으로 호출한다.
