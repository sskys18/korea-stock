...
use crate::domestic::kis::realtime::decode::{self, ControlMessage, DecodedRecord};
use crate::domestic::kis::realtime::subscribe::{build_frame, ControlMsg};
use crate::domestic::kis::realtime::{RealtimeEvent, SubMap};
...
async fn run_one_connection(ctx: &mut ConnectionCtx, url: &str) -> ConnEnd {
    let approval_key =
        match crate::domestic::kis::realtime::approval::issue_approval_key(&ctx.http, &ctx.config).await {
            Ok(k) => k,
            Err(e) => {
    ...
    let mut creds: std::collections::HashMap<String, crate::domestic::kis::realtime::crypto::AesCreds> =
...
async fn handle_text<S>(
    ctx: &ConnectionCtx,
    sink: &mut S,
    creds: &mut std::collections::HashMap<String, crate::domestic::kis::realtime::crypto::AesCreds>,
    text: &str,
) -> std::result::Result<(), ()>
where
