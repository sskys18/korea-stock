...
//!
//! Aster V1(Legacy) fapi의 Binance 호환 HMAC-SHA256 서명 경로. **V3 EIP-712
//! 온체인 서명 경로는 이 모듈이 구현하지 않는다.**
//!
//! **실거래 경고:** 이 모듈의 모든 호출은 실제 자금을 움직인다. Aster는 BNB
//! Chain 정산 perp DEX이므로 운영 전 소액으로 검증할 것.

use serde::Deserialize;

use crate::global::aster::client::{ApiCall, AsterClient};
use crate::global::aster::error::Result;
