...
//! [`crate::hyperliquid::HyperliquidConfig::testnet`]에서 한다(단, HIP-3 `xyz` dex가
//! 테스트넷에 존재하는지 먼저 확인).
//!
//! ## wire 필드 순서는 서명의 일부
...
use crate::hyperliquid::client::{parse_json, HyperliquidClient};
...
pub struct OrderRequest {
    /// asset 정수 id. KR 종목은 [`crate::hyperliquid::SAMSUNG_ASSET`] 등.
    pub asset: u64,
    pub is_buy: bool,
...
impl<'a> Trade<'a> {
    ...
    /// ([`crate::hyperliquid::SAMSUNG_ASSET`] 등)는 universe 재정렬로 틀어질 수 있다 —
    ...
    /// coin 이름(예 [`crate::hyperliquid::SAMSUNG`])으로 지정가 주문.
    ///
    /// asset id를 **주문 직전 라이브 meta에서 재도출**([`crate::hyperliquid::market::Market::asset_id`])
    ...
    pub async fn place_by_coin(
        ...
        let asset = crate::hyperliquid::market::Market::new(self.client)
            .asset_id(dex, coin)
            .await?;
    ...
    /// 종목 포지션은 `dex`(예 [`crate::hyperliquid::DEX`])를 줘야 보인다.
...
mod tests {
    use super::*;
    use crate::hyperliquid::client::sign_l1_action;
