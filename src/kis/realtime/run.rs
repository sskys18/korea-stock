...
use crate::kis::realtime::decode::{self, ControlMessage, DecodedRecord};
use crate::kis::realtime::subscribe::{build_frame, ControlMsg};
use crate::kis::realtime::{RealtimeEvent, SubMap};
...
async fn run_one_connection(ctx: &mut ConnectionCtx, url: &str) -> ConnEnd {
    let approval_key =
        match crate::kis::realtime::approval::issue_approval_key(&ctx.http, &ctx.config).await {
            Ok(k) => k,
            Err(e) => {
    ...
    let mut creds: std::collections::HashMap<String, crate::kis::realtime::crypto::AesCreds> =
...
async fn handle_text<S>(
    ctx: &ConnectionCtx,
    sink: &mut S,
    creds: &mut std::collections::HashMap<String, crate::kis::realtime::crypto::AesCreds>,
    text: &str,
) -> std::result::Result<(), ()>
where
