...
use crate::global::lighter::client::{ApiCall, LighterClient};
use crate::global::lighter::error::{LighterError, Result};
use crate::global::lighter::sign;
...
pub struct OrderRequest {
    /// 마켓 식별자. 심볼→id는 [`crate::global::lighter::market::Market::market_id`].
    pub market_id: u32,
    pub side: Side,
...
impl<'a> Trade<'a> {
    ...
    fn guard_signing(&self) -> Result<&crate::global::lighter::config::LighterConfig> {
        self.ensure_signer()?;
        let cfg = self.client.config();
...
fn random_scalar() -> crate::global::lighter::sign::scalar::Scalar {
    let mut buf = [0u8; 40];
    getrandom::getrandom(&mut buf).expect("OS CSPRNG failed — refusing to sign with weak nonce");
    crate::global::lighter::sign::scalar::Scalar::from_le_bytes(&buf)
}
