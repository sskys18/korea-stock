...
async fn balance_query() {
    ...
    let (_items, summary) = ds
        .balance_all(korea_stock::domestic::kis::domestic_stock::BalanceBasis::Default)
        .await
        .expect("balance call");
...
async fn overseas_current_price() {
    ...
    let price = os
        .current_price(korea_stock::domestic::kis::overseas_stock::OverseasExchange::Nasd, "AAPL")
        .await
...
async fn futureoption_current_price() {
    ...
    let (price, raw) = fo
        .current_price(korea_stock::domestic::kis::futureoption::MarketDiv::IndexFuture, "101W09")
        .await
...
async fn nxt_unified_support_audit() {
    use korea_stock::domestic::kis::domestic_stock::{MarketClass, Period, RankBy};
    let client = client().expect("KIS_* env vars");
    let ds = client.domestic_stock();
...
async fn domestic_period_price() {
    use korea_stock::domestic::kis::domestic_stock::Period;
    let client = client().expect("KIS_* env vars");
...
async fn domestic_buyable() {
    use korea_stock::domestic::kis::domestic_stock::OrderType;
    let client = client().expect("KIS_* env vars");
    let info = client
        .domestic_stock()
...
async fn domestic_daily_conclusions() {
    use korea_stock::domestic::kis::domestic_stock::SellBuy;
    let client = client().expect("KIS_* env vars");
    let page = client
        .domestic_stock()
...
async fn overseas_conclusions() {
    use korea_stock::domestic::kis::overseas_stock::{FilledFilter, OverseasSellBuy};
    let client = client().expect("KIS_* env vars");
    let list = client
        .overseas_stock()
        .conclusions_all(
...
async fn futureoption_conclusions() {
    use korea_stock::domestic::kis::futureoption::{CcnlFilter, CcnlSellBuy};
    let client = client().expect("KIS_* env vars");
    let r = client
        .futureoption()
...
async fn futureoption_buyable() {
    use korea_stock::domestic::kis::futureoption::SellBuy;
    let client = client().expect("KIS_* env vars");
    let r = client
        .futureoption()
